...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Processa max 2 por execução (limite 10s Vercel Hobby)
    const toProcess = raw.filter(item => !existingIds.has(item.external_id)).slice(0, 2);
    ...
    await supabaseAdmin.from('digest_runs').insert({
      run_date: since, events_fetched: fetched, events_new: inserted, status: 'ok',
    });
