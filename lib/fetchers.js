...
const KEYWORDS = ['embalagem', 'plástico', 'celulose', 'reciclagem', 'logística reversa'];
function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }
function getLastWeekISO() { const d = new Date(); d.setDate(d.getDate() - 30); return d.toISOString().split('T')[0]; }
  ...
  for (const kw of KEYWORDS) {
    try {
      ...
      for (const prop of (data.dados || [])) {
        results.push({ external_id: `camara-${prop.id}`, source: 'dadosabertos.camara.leg.br', source_url: `https://www.camara.leg.br/proposicoesWeb/fichadetramitacao?idProposicao=${prop.id}`, raw: `${prop.siglaTipo} ${prop.numero}/${prop.ano} — ${prop.ementa}` });
      }
      await sleep(400);
...
async function fetchSenadoMaterias(since) {
  ...
  for (const kw of ['embalagem', 'plástico', 'reciclagem']) {
    try {
      const res = await fetch(`${SENADO_BASE}/materia/pesquisa-basica?palavraChave=${encodeURIComponent(kw)}&dataInicio=${since}&situacao=tramitando`, { headers: { Accept: 'application/json' } });
      if (!res.ok) continue;
      const data = await res.json();
      ...
      for (const m of list) {
        ...
        results.push({ external_id: `senado-${m.CodigoMateria}`, source: 'legis.senado.leg.br', source_url: `https://www25.senado.leg.br/web/atividade/materias/-/materia/${m.CodigoMateria}`, raw: `${m.SiglaTipoMateria} ${m.NumeroMateria}/${m.AnoMateria}\n${m.EmentaMateria}` });
      }
      await sleep(500);
...
async function fetchDOU(since) {
  const results = [];
  for (const q of ['embalagem plástico', 'logística reversa embalagem']) {
    try {
      ...
      for (const g of (data.gazettes || [])) {
        for (const excerpt of (g.excerpts || []).slice(0, 2)) {
          ...
          results.push({ external_id: `dou-${g.date}-${hash}`, source: 'Diário Oficial da União', source_url: g.url, raw: `DOU ${g.date}\n\n${excerpt}` });
        }
      }
      await sleep(400);
...
async function fetchAllSources(since) {
  const [camara, senado, dou] = await Promise.allSettled([fetchCamaraProposicoes(since), fetchSenadoMaterias(since), fetchDOU(since)]);
  return [...(camara.value || []), ...(senado.value || []), ...(dou.value || [])];
}
